...
def build_prompt(ticker, interval, signals, signal, confidence,
    ...
    if earnings.get('warning'):
        earnings_warning = f"""
⚠️ EARNINGS WARNING: Earnings in {earnings.get('days_until')} days ({earnings.get('earnings_date')})
...
FINAL CALL: [TAKE THE TRADE ✅ / WAIT FOR BETTER SETUP ⚠️ / AVOID ❌]

VETERAN NOTES:
[2-3 sentences of hard-earned wisdom specific to THIS setup right now.
Speak like someone who has traded for 20 years and has seen this exact pattern before.]"""
...
if __name__ == "__main__":
    ...
    alt_strat= get_alternative_strategy(best_opt, data['options'], signal, signals['close'])
    ...
    swing    = calculate_swing_score(signals, market, sector, data['options'],
